...
def _send_email(job: NotificationJob, config: NotificationChannelConfig) -> dict[str, Any]:
    from_email = config.sender_identifier or getattr(settings, "DEFAULT_FROM_EMAIL", "no-reply@gepub.local")
    ...
    send_mail(subject=subject, message=message, from_email=from_email, recipient_list=[job.destination], fail_silently=False)
    return {"provider": config.provider, "result": "accepted", "to": job.destination}
